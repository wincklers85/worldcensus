...
const fallbackMakes = Object.keys(typeof LOCAL_CATALOGUE !== 'undefined' ? LOCAL_CATALOGUE : {}).length ? Object.keys(LOCAL_CATALOGUE) : ['Abarth', 'Audi', 'BMW', 'Citroen', 'Fiat', 'Ford', 'Honda', 'Mercedes-Benz', 'Mini', 'Peugeot', 'Porsche', 'Renault', 'Seat', 'Skoda', 'Smart', 'Tesla', 'Toyota', 'Volkswagen'];
...
function getLocalMakes() {
  return uniqueSorted(Object.keys(typeof LOCAL_CATALOGUE !== 'undefined' ? LOCAL_CATALOGUE : {}));
}

function getLocalModels(make) {
  return uniqueSorted((typeof LOCAL_CATALOGUE !== 'undefined' ? (LOCAL_CATALOGUE[make] || []) : []));
}

async function loadMakes() {
  const localMakes = getLocalMakes();
  state.liveMakes = localMakes.length ? localMakes : fallbackMakes;
  ...
  setStatus(localMakes.length ? 'Catalogo marche pronto' : 'Catalogo marche pronto (fallback)', true);
}
...
async function loadModelsForMake(make) {
  ...
  const localModels = getLocalModels(make);
  const merged = uniqueSorted([...localModels, ...getCuratedModels(make)]);
  state.makeModelsCache[make] = merged;
  state.currentModels = [...merged];
  state.filteredModels = [...merged];
  populateSelect(els.modelSelect, state.filteredModels, merged.length ? 'Seleziona un modello' : 'Nessun modello disponibile');
  els.modelSelect.disabled = merged.length === 0;
  els.modelFilterInput.disabled = merged.length === 0;
  setStatus(merged.length ? `Modelli pronti per ${make}` : `Nessun modello disponibile per ${make}`, merged.length > 0);
}
...
async function init() {
  ...
  attachEvents();
  await loadMakes();
  setStatus('Catalogo locale caricato', true);
}
